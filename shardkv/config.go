...
func (cfg *config) checklogs() {
	for gi := 0; gi < cfg.ngroups; gi++ {
		for i := 0; i < cfg.n; i++ {
			omnipaxos := cfg.groups[gi].saved[i].OmnipaxosStateSize()
			snap := len(cfg.groups[gi].saved[i].ReadSnapshot())
			if cfg.maxpaxosstate >= 0 && omnipaxos > 8*cfg.maxpaxosstate {
				cfg.t.Fatalf("persister.OmniPaxosStateSize() %v, but maxpaxosstate %v",
					omnipaxos, cfg.maxpaxosstate)
			}
			if cfg.maxpaxosstate < 0 && snap > 0 {
				cfg.t.Fatalf("maxpaxosstate is -1, but snapshot is non-empty!")
			}
		}
	}
}
...
func (cfg *config) StartServer(gi int, i int) {
	...
	if gg.saved[i] != nil {
		...
		gg.saved[i] = omnipaxos.MakePersister()
	}
	cfg.mu.Unlock()

	gg.servers[i] = StartServer(ends, i, gg.saved[i], cfg.maxpaxosstate,
		gg.gid, mends,
	...
	rfsvc := labrpc.MakeService(gg.servers[i].op)
...
func make_config(t *testing.T, n int, unreliable bool, maxpaxosstate int) *config {
	...
	for gi := 0; gi < cfg.ngroups; gi++ {
		...
		gg.saved = make([]omnipaxoslib.Persistable, cfg.n)
